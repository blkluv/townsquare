...
import {BundledFn} from '../../lib/async/bundle'
...
interface ActionsGlobals {
  ...
  actionKeyToExecutingActionId: {
    [actionKey: string]: {[id: number | string]: 1}
  }
...
export class SplxActionModel {
  ...
  private unsetCurrentExectutingActionId(_id: number) {
    runInAction(() => {
      this.globals.currentExecutingActionId = 0
    })
  }

  private getKeyPrefix<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    return `${statePayload.store.name}/${statePayload.actionName}#`
  }

  private getActionKeyFromPayload<Args extends readonly unknown[]>(
    payload: Args,
  ): string | undefined {
  ...
  private maybeWarnPayload<Args extends readonly unknown[]>(
    ...
    if (
      payloadStr.length < 16384 ||
      this.globals.alreadyWarned[prefix] ||
      !isLocal()
    ) {
      return
    }
  ...
  private getPayloadString<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
    prefix?: string,
  ): string {
    prefix = prefix ?? this.getKeyPrefix(statePayload)
    const str =
      this.getActionKeyFromPayload(statePayload.payload) ??
      JSON.stringify(statePayload.payload ?? '')
    this.maybeWarnPayload(prefix, str, statePayload)
    return str
  }

  private payloadToKey<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
    prefix?: string,
  ) {
    prefix = prefix ?? this.getKeyPrefix(statePayload)
  ...
  private getActionKey<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args> | string,
  ): string {
  ...
  private getActionState<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args> | string,
  ) {
  ...
  private setIdle<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args> | string,
  ) {
  ...
  private createActionState<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
    key?: string,
  ) {
    ...
    if (this.getActionState(key)) {
      console.warn(
        `Trying to re-create an action state for an action that is already there (${key})`,
      )
      return
    }
  ...
  private setBusy<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    const key = this.getActionKey(statePayload)
  ...
  private setError<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args> | string,
    error: Error,
  ) {
    const key = this.getActionKey(statePayload)
  ...
  private setErrorsForStatePayload<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
    err: Error,
  ) {
    const actionKey = statePayload.key as string
    const actionIdsToWalk = {
      ...(this.globals.actionKeyToExecutingActionId[actionKey] ?? {}),
    }
        ...
        for (let i = keys.length - 1; i >= 0; i--) {
          ...
          if (!setKeys[key]) {
            ...
            const actionIds = Object.keys(
              this.globals.actionKeyToExecutingActionId[key] ?? {},
            )
  ...
  private setExecutingAction<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    ...
    if (!this.globals.executingActions[statePayload.executingActionId]) {
      runInAction(() => {
        this.globals.executingActions[
          statePayload.executingActionId as number
        ] = []
      })
    }
    ...
    runInAction(() => {
      ...
      this.globals.executingActions[
        statePayload.executingActionId as number
      ].push(statePayload.key as string)
    })
  }

  private unsetExecutingAction<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
    executingActionId: number,
  ) {
  ...
  private getError<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
  ...
  private getIsLoading<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    const actionState = this.getActionState(statePayload)
  ...
  private getIsBusy<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    const actionState = this.getActionState(statePayload)
    return (
      actionState?.status === ActionStatus.Busy ||
      this.getIsLoading(statePayload)
    )
  }

  private getIsIdle<Args extends readonly unknown[]>(
    statePayload: ActionStatePayload<Args>,
  ) {
    const actionState = this.getActionState(statePayload)
  ...
  private actionGetter<Args extends readonly unknown[], Res>(
    status: ActionStatus | 'ActionKey',
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  error<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  isBusy<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  isIdle<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  isLoading<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  isNew<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  actionKey<Args extends readonly unknown[], Res>(
    fn: ((...args: Args) => Promise<Res>) | string,
    context: any,
    args: Args,
  ) {
  ...
  wrapAction<Args extends readonly unknown[], Res>(
    ...
    const wrappedAction = async (
      ...
      return await new Promise((resolve, reject) => {
        ...
        ;(fn as (...args: Args) => Promise<Res>)
          .apply(
            actionStatePayload.store,
            actionStatePayload.payload as unknown as any,
          )
          .then(result => {
            this.setCurrentExecutingActionId(executingActionId)
