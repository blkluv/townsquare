...
export function getAuthorId<O extends { author?: string; uri?: string }>({ author, uri }: O) {
  if (!author && uri) {
...
function mockReward(isWeekly: boolean = false) {
  return isWeekly ? {
    image: 'https://splx-prod.s3.amazonaws.com/reactions/gaming/images/bruh.png',
    symbol: 'RBNG',
    name: 'Rubian Bruh Reaction',
    description: 'The Rubian Reaction Pack',
    attributes: { artist: '@DarknessPixie', trait: 'bruh', splx_type: 'Solarplex Reaction' },
    image_file: 'Olami_bruh.png',
  } : {
    image: 'https://splx-prod.s3.amazonaws.com/reactions/gaming/images/salute.png',
    symbol: 'RBNG',
    name: 'Rubian Salute Reaction',
    description: 'The Rubian Reaction Pack',
    attributes: { artist: '@DarknessPixie', trait: 'salute', splx_type: 'Solarplex Reaction' },
    image_file: 'Olami_salute.png',
  }
}

function mockClaim(shouldClaim: boolean, isClaiming: boolean, claimed: boolean, isWeekly: boolean) {
  return {
    shouldClaim,
    isClaiming,
    claimed,
...
function getMockMissions(did: string, percents: number[]) {
  ...
  for (let i = 0; i < missions.length; i++) {
    ...
    const claim = mockClaim(shouldClaim, shouldClaim && claiming, shouldClaim && !claiming && claimed, isWeekly);
...
export class RewardsModel {
  ...
  _claimRewards = actions.wrapAction(async (userId: string, wallet: string, missionIds: string[]) => {
    ...
    const url = `${SOLARPLEX_V1_API}${apiUrls.rewards.postClaimReward(userId)}`
    const body = {
      mission: {
    ...
    const mockingDaily = mock && missionIds.includes(this.dailyMissionId(userId));
    const mockingWeekly = mock && missionIds.includes(this.weeklyMissionId(userId));
    if (mockingDaily) {
      claimingDaily = true;
    }
    if (mockingWeekly) {
  ...
  dailyMissionsToClaim(userId: string) {
    return this.dailyMissions(userId).filter((i) => i.shouldClaim && !this.isClaimingMission(userId, i.id));
  }

  weeklyMissionsToClaim(userId: string) {
    const mission = this.weeklyMission(userId);
    if (!mission || !mission.shouldClaim || this.isClaimingMission(userId, mission.id)) {
      return [];
    }
    return [mission];
  }
  ...
  isClaimingMission(userId: string, missionId: string) {
    ...
    return !!inFlightClaims[missionId] || mission?.isClaiming || this.isClaimWaitingOnBlockchain(userId, missionId);
  }
  ...
  shouldClaimWeekly(userId: string): boolean | undefined {
    return (
      this.weeklyMission(userId)?.shouldClaim && !this.shouldClaimDaily(userId) && !this.isClaimingWeekly(userId)
    );
  }
  ...
  async claimDailyReward(userId: string) {
    ...
    return await this._claimRewards(userId, this.rootStore.me.splxWallet, missionIds);
  }
  ...
  _fetchMissions = actions.wrapAction(async (userId: string, meId: string) => {
    runInAction(() => {
